...
export const cssColorNames: Record<string, [number, number, number]> = {
...
export function rgb2hex([r, g, b]: [number, number, number]): number {
	return (r << 16) + (g << 8) + b;
}
...
function _interpolateVector(a: Vector2, b: Vector2, t: number): Vector2 {
	return new Vector2(a.x * (1 - t) + b.x * t, a.y * (1 - t) + b.y * t);
}
function _interpolateColor(a: [number, number, number], b: [number, number, number], t: number): [number, number, number] {
...
function _interpolateArray<T>(arr: Array<T>, t: number, interpolateFn: (a: T, b: T, t: number) => T) {
	if (!Array.isArray(arr) || arr.length === 0) throw { message: "got empty array" };
...
export function interpolate<T>(arr: Array<T>, t: number): T {
	if (typeof arr[0] === "number") {
		return _interpolateArray(arr as number[], t, _interpolateNumber) as T;
	} else if (arr[0] instanceof Vector2) {
		return _interpolateArray(arr as Vector2[], t, _interpolateVector) as T;
	} else if (Array.isArray(arr[0]) && typeof arr[0][0] === "number") {
		return _interpolateArray(arr as [number, number, number][], t, _interpolateColor) as T;
	} else {
		throw { message: "Not implemented" };
	}
}
export function arrayFirstOrDefault<T>(arr?: Array<T>, value?: T) {
	return arr?.[0] ?? (value as T);
}
export function getRandomPointInCircle(cx: number, cy: number, radius: number): { x: number; y: number } {
...
export function getRandomPointOnCircumference(cx: number, cy: number, radius: number): { x: number; y: number } {
	const angle = 2 * Math.PI * Math.random(); // Random angle from 0 to 2π
	const x = cx + radius * Math.cos(angle);
	const y = cy + radius * Math.sin(angle);
	return { x, y };
}
export function getRandomPointOnRectangleEdge(x: number, y: number, width: number, height: number): { x: number; y: number } {
